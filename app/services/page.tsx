...
export default function Services() {
  ...
  return (
    <div className='w-full h-full flex-1 justify-center items-center bg-[#272C35] md:px-20 px-6 md:py-20 py-6'>
      ...
      <div className='grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 md:gap-10 gap-5'>
        {serviceDetails.map((service, index) => (
          <div key={index} className='md:w-[386px] md:h-[260px] w-full h-52 rounded-[7px] p-6 bg-[#1A1D23] flex flex-col md:gap-4 gap-2'>
            <div className='md:w-14 md:h-14 w-10 h-10 rounded bg-gradient-to-r from-[#FA7E16] to-[#FE9619] items-center justify-center flex md:text-2xl text-base'>
              <h1 className='text-[#FAFAFA] text-2xl'>{service.icon}</h1>
            </div>
